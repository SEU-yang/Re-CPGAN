...
class GeneratorResNet(nn.Module):
    ...
    def forward(self, fc, fs, num_modules):
        ...
        for _ in range(16):
            a, attention, b = self.ex_cpnet(a, b)

            heatmap_fc = self.fan(a)  ## facial landmark heatmaps of content faces  68*64*64
            heatmap_fs = self.fan(b)  ## facial landmark heatmaps of style faces  68*64*64
            gen_fan_fc = torch.cat(heatmap_fc, 0)
            gen_fan_fs = torch.cat(heatmap_fs, 0)

            fc_64_concat = torch.cat([a, gen_fan_fc], 1)  ## concatenates content features 196*64*64
            fs_64_concat = torch.cat([b, gen_fan_fs], 1)  ## concatenates style features 196*64*64

            a = self.conv2(fc_64_concat)  ## shrunken content features 128*128*128
            b = self.conv2(fs_64_concat)  ## shrunken style features 128*128*128

            a = torch.add(a, x_incp_FS)
            b = torch.add(b, x_fs_FS)

        fcout = self.res_block_out(a)  ## enhanced content features 128*128*128

        fsout = self.res_block_out(b)  ## enhanced style features 128*128*128

        out_fc1 = self.conv3(fcout)  ## output content features 32*128*128

        out_fs1 = self.conv3(fsout)  ## output style features 32*128*128

        out_fc = self.conv4(out_fc1)  ## output content features 3*128*128

        out_fs = self.conv4(out_fs1)  ## output style features 3*128*128
